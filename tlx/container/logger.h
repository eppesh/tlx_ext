...
class PosixLogger {
   public:
    ...
    void Logv(const char* format, va_list arguments) {
        ...
        static_assert(
            sizeof(stack_buffer) == static_cast<size_t>(kStackBufferSize),
        ...
        for (int iteration = 0; iteration < 2; ++iteration) {
            ...
            // 3 spacing) plus the thread ID, which should fit comfortably into
            // the static buffer.
            assert(buffer_offset <= 28 + kMaxThreadIdSize);
            static_assert(
                28 + kMaxThreadIdSize < kStackBufferSize,
            ...
            if (buffer_offset >= buffer_size - 1) {
                ...
                // (v)snprintf. Fail in tests, recover by truncating the log
...
#define INFO(M, ...)                                                   \
    do {                                                               \
        ...
        sprintf(buffer + 8, "[%s %s:%d] ", __FILENAME__, __FUNCTION__, \
                __LINE__);                                             \
...
#define WARNING(M, ...)                                               \
    do {                                                              \
        ...
        sprintf(buffer + strlen(buffer), "[%s %s:%d] ", __FILENAME__, \
